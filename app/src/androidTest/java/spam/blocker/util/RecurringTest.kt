...
class RecurringTest {
    ...
    fun weekly() {
        ...
        setNow(LocalDateTime.parse("2024-10-1T0:0:0"))
        ...
        assertEquals("next week", 7*24*3600*1000, dur.toMillis())

        // now: <2024-10-31 Thursday 0:0:0>, weekdays: [Thursday], time: <0:0:0>  -> dur: 7 days
        setNow(LocalDateTime.parse("2024-10-31T0:0:0"))
        dur = Weekly(listOf(THURSDAY), Time(0,0)).nextOccurrence()
        assertEquals("cross month", 24*3600*1000, dur.toMillis())

        // now: <2024-12-31 Tuesday 0:0:0>, weekdays: [Tuesday], time: <0:0:0>  -> dur: 7 days
        setNow(LocalDateTime.parse("2024-12-31T0:0:0"))
        dur = Weekly(listOf(TUESDAY), Time(0,0)).nextOccurrence()
        assertEquals("cross year", 24*3600*1000, dur.toMillis())
    }
}